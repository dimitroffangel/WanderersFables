...
exports.forgeMenu = function(key){
    ...
    if(key.name == 'return' && lastKey != 'return'){
        if(forgeChosenOption == 'Created decks' && playerDecks[forgeIndex]){
    ...
    // writing the deck's name
            ...
            key.name != 'up' && key.name != 'space' && key.name)
        deckName += key.name;
        ...
        if(forgeChosenOption == 'Create a deck'){
            ...
            playerDecks[forgeIndex].isUsable = true;
        }
...
function printIndex(){
    ...
    if(forgeChosenOption == 'Rebuild a deck'){
        ...
        ctx.point(0,2, onPage + '|On card: ' + (forgeIndex + 1) + 
        '| Cards in deck: ' + playerDecks[playerDecks.length-1].deck.length);
    }
}
...
function createDeck(key){
    ...
    if(key.name == 'up' && forgeIndex > 0)
        ...
        var cardsNames = variables.encodeDeck(playerDecks[indexOnDeck].deck);

        socket.emit('CreatedDeck',{username:profileUsername,userID: userID, 
                                   deck: cardsNames,deckName:playerDecks[indexOnDeck].name,
                                   isUsable:playerDecks[indexOnDeck].isUsable});
